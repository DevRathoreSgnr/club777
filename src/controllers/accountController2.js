...
if (rows.length == 0) {
    try {
        const response = await axios.get('https://www.fast2sms.com/dev/bulkV2', {
            params: {
                authorization: 'oDIebd2WkulT7AZKLJ49VEwQSvcXPnf0Y3mxi5MyzpBRgHj1OFl8bTt6xdc5ohKILG4QsUeSWXRwDF31',
                sender_id: 'FTWSMS',
                route: 'v3',
                language: 'english',
                flash: 0,
        ...
        // Check for success based on the message content
        if (response.data && Array.isArray(response.data.message) && response.data.message.includes('SMS sent successfully.')) {
            ...
            // Log the response message if the OTP sending failed
        ...
        if (user.time_otp - now <= 0) {
            request(`https://www.fast2sms.com/dev/bulkV2?authorization=oDIebd2WkulT7AZKLJ49VEwQSvcXPnf0Y3mxi5MyzpBRgHj1OFl8bTt6xdc5ohKILG4QsUeSWXRwDF31&sender_id=FTWSMS&route=v3&language=english&flash=0&numbers=${phone}&message=${otp}`,  async(error, response, body) => {
