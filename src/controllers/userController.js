...
const recharge = async (req, res) => {
    ...
    if (recharge.length == 0) {
        ...
        const apiData = {
            key: "980f5dc9-8781-467b-89d9-efaf6f81dde3",
            client_txn_id: client_transaction_id,
            amount: moneyString,
            p_info: 'WinCash Recharge',
            customer_name: userInfo.name_user,
            customer_email: 'contact@91club.in',
            customer_mobile: userInfo.phone,
            redirect_url: "/wallet/rechargerecord",
            udf1: 'Fairclub',
        };
        try {
...
const withdrawal3 = async (req, res) => {
    ...
    let money = req.body.money;
    let password = req.body.password;
    if (!auth || !money || !password || money < 499) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
            timeStamp: timeNow,
